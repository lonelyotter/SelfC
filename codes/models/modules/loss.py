...
class MotionFlowLoss(nn.Module):
    ...
    def forward(self, x_lr, target_hr):
        ...
        # exit()
        target_mv = torch.nn.functional.upsample(target_mv,
                                                 scale_factor=(0.25, 0.25),
                                                 mode='area')
        lr_mv = self.opticFlow_Net(x_lr_2, x_lr_1)
        target_mv = torch.cat([
            target_mv[:, 0:1, :, :] / ((target_mv.size(3) - 1.0) / 2.0),
            target_mv[:, 1:2, :, :] / ((target_mv.size(2) - 1.0) / 2.0)
        ], 1)
        lr_mv = torch.cat([
            lr_mv[:, 0:1, :, :] / ((lr_mv.size(3) - 1.0) / 2.0),
            lr_mv[:, 1:2, :, :] / ((lr_mv.size(2) - 1.0) / 2.0)
        ], 1)
