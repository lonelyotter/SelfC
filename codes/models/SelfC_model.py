...
class SelfCModel(BaseModel):
    ...
    def feed_data(self, data):
        '''
        load input video to the model and generate reference LR video

        Args:
            data: input video data, size: b, C, T, H, W
        
        Returns:
            length of current video clip
        '''
        ...
        if "LQ" in data: # low resolution video is provided
            ...
            elif self.opt["distortion"] == "sr_bd":  # our project's situation
                self.ref_L = Guassian_downsample(self.real_H.transpose(
                    0, 1)).transpose(0, 1)

            elif self.opt["distortion"] == "matlab":
                self.ref_L = matlab_lr.imresize(self.real_H,
