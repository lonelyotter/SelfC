...
class LQGTVIDDataset(data.Dataset):
    ...
    def __init__(self, opt):
        '''
		Initialization for LQGTVIDDataset.

		Args:
    		opt: options for LQGTVIDDataset.
		'''
        super(LQGTVIDDataset, self).__init__()
        self.opt = opt

        # data type (img | lmdb), in this project, we only use img
        ...
        self.paths_GT, self.sizes_GT = util.get_vid_paths(
            self.data_type, opt['dataroot_GT'], opt['dataroot_list'],
            GlobalVar.get_Istrain())

        if self.opt['phase'] != 'train' and opt[
                "sample_num"] and opt["sample_num"] > 0:
    ...
    def _init_lmdb(self):
        ...
        self.GT_env = lmdb.open(self.opt['dataroot_GT'],
                                readonly=True,
                                lock=False,
                                readahead=False,
                                meminit=False)
        self.LQ_env = lmdb.open(self.opt['dataroot_LQ'],
                                readonly=True,
                                lock=False,
                                readahead=False,
                                meminit=False)
    ...
    def gen_aug_params(self):
        '''
        Generate augmentation parameters. Include random flip, rotation.
        '''
    ...
    def read_img(self, GT_path, scale, GT_size):
        ...
        Args:
            GT_path (str): path of GT image
            scale (int): useless
            GT_size (int): size of GT image
        
        Returns:
            img_GT (torch.Tensor): GT image, CHW, RGB, [0,1], torch.float32
        '''
        resolution = None
        ...
        if self.opt['phase'] == 'train':
            ...
            [img_GT] = util.augment([img_GT], self.hflip, self.vflip,
                                    self.rot90)
            ...
            # change from HWC to CHW, numpy to tensor
    ...
    def __getitem__(self, index):
        '''
        Returns a GT video and its metadata information of the dataset.
        
        Args:
			index (int): Index
        
        Returns:
            dict: GT video and its metadata information.
        '''
        import random
        ...
        scale = self.opt['scale'] # useless
        GT_size = self.opt['GT_size']
        video_len = self.opt['video_len']

        # get GT image
        GT_path = self.paths_GT[index] # paths of GT images in a video
        vid_HQ = []
        vid_LQ = []
        self.gen_aug_params()
        ...
        if video_len == 5:
            if GT_path_len > 5:
                ...
                if self.opt['phase'] == "train":
                    ...
                    GT_paths = [
                        GT_path[index1], GT_path[index2], GT_path[index3],
                        GT_path[index4], GT_path[index5]
                    ]
